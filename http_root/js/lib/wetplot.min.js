...
for(var DEFAULT_LINE_CODE="###default###",WETPLOT_CSS="\n    .xAxisText {\n        fill: #000000;\n    }\n\n    .linePath {\n        fill: none;\n    }\n\n    .grid {\n        fill: none;\n        stroke: #666;\n    }\n\n    #hoverCursor {\n        stroke: #000;\n    }\n\n    .wetplot-button {\n        height: 2rem;\n        width: 2rem;\n        font-size: 1.8rem;\n        background-color: #aaaaaaaa;\n        text-align: center;\n        vertical-align: center;\n        margin-top: 0.25rem;\n        margin-left: 0.25rem;\n    }\n\n    .wetplot-button:hover {\n        background-color: #ccccccaa;\n    }\n\n    .wetplot-button:not(:last-child) {\n        margin-right: 0.25rem;\n    }\n\n    #wetplot-button-container {\n        z-index: 100;\n    }\n",COLOR_PALETTES=
[["#ff0000","#00ff00","#00ffff","#0000ff","#ff00ff"],"#ff0000 #ff8000 #80ff00 #00ff00 #00ff80 #00ffff #0080ff #0000ff #8000ff #ff00ff #ff0080".split(" ")],TIMEDELTA_UNITS=[["a",52],["w",7],["d",24],["h",60],["m",60],["s",1]],i=TIMEDELTA_UNITS.length-2;0<=i;i--)TIMEDELTA_UNITS[i][1]*=TIMEDELTA_UNITS[i+1][1];var TEXTS={TIME:{en:"Time",de:"Zeit",fr:"Temps"},VISIBLE_LINES:{en:"Visible Lines",de:"Sichtbare Linien",fr:"Lignes visibles"},YBAR_WIDTH:{en:"Vertical Bar Width",de:"S\u00e4ulenbreite"}};
...
function timedeltaSecondsToHumanReadable(a){a=void 0===a?0:a;if(0>a)throw Error("Negative delta not supported yet");for(var b=[],c=$jscomp.makeIterator(TIMEDELTA_UNITS),d=c.next();!d.done;d=c.next()){var e=$jscomp.makeIterator(d.value);d=e.next().value;e=e.next().value;a>=e&&(b.push(Math.floor(a/e)+d),a%=e)}return b.join(" ")}
...
ybar_width_seconds:3600};this._line_config={"###default###":{type:"line",color:"#000000",line_width:1,name:"?",unit:"1",auto_min_max:!1,min:0,max:10,visible:!0}};this._data=null;this._x_offset=0;this._data_callback=null};
...
Wetplot.prototype._set_auto_min_max=function(){var a=this,b=[],c={},d=this._data.heads.indexOf("Time");this.getVisibleLineCodes().forEach(function(p){var q=a._line_config[p].unit;-1===b.indexOf(q)?(b.push(q),c[b.length-1]=[p]):c[b.indexOf(q)].push(p)});for(var e={},f=0;f<b.length;e={$jscomp$loop$prop$min$29:e.$jscomp$loop$prop$min$29,$jscomp$loop$prop$max$30:e.$jscomp$loop$prop$max$30},f++){var g=c[f],h=g.filter(function(p){return a._line_config[p].auto_min_max});e.$jscomp$loop$prop$min$29=Infinity;
e.$jscomp$loop$prop$max$30=-Infinity;g=$jscomp.makeIterator(g);for(var l=g.next();!l.done;l=g.next()){var k=l.value;if(this._line_config[k].auto_min_max){var m=this._line_config[k].type;l=this._data.heads.indexOf(k);if("line"===m)k=$jscomp.makeIterator(this._data.getMinMaxForColumn(l)),l=k.next().value,k=k.next().value,e.$jscomp$loop$prop$min$29=Math.min(e.$jscomp$loop$prop$min$29,l),e.$jscomp$loop$prop$max$30=Math.max(e.$jscomp$loop$prop$max$30,k);else if("ybar"===m){k=void 0;for(var n=m=0;n<this._data.values.length;n++){var r=
this.changingPropertyForYBarLines(this._data.values[n][d]);void 0!==k&&k!==r&&(e.$jscomp$loop$prop$min$29=Math.min(e.$jscomp$loop$prop$min$29,m),e.$jscomp$loop$prop$max$30=Math.max(e.$jscomp$loop$prop$max$30,m),m=0);m+=this._data.values[n][l];k=r}}else throw Error('unsupported line type "'+m+'" for line "'+k+'"');}else e.$jscomp$loop$prop$min$29=Math.min(e.$jscomp$loop$prop$min$29,this._line_config[k].min),e.$jscomp$loop$prop$max$30=Math.max(e.$jscomp$loop$prop$max$30,this._line_config[k].max)}h.forEach(function(p){return function(q){a._line_config[q].min=
p.$jscomp$loop$prop$min$29;a._line_config[q].max=p.$jscomp$loop$prop$max$30}}(e))}};
...
Wetplot.prototype._redraw_y_axis=function(){var a=this,b=this._config.font_size_px,c=this._get_visible_units();this._yAxisElement.setAttribute("height",this._config.height);this._yAxisElement.setAttribute("width",4*c.length*b+"px");this._yAxisElement.style.backgroundColor=this._config.background_color;this._yAxisElement.innerHTML="";for(var d={},e=0;e<c.length;d={$jscomp$loop$prop$unit$32:d.$jscomp$loop$prop$unit$32},e++){d.$jscomp$loop$prop$unit$32=c[e];var f="group"+this._unit_to_id(d.$jscomp$loop$prop$unit$32),
g=this.getVisibleLineCodes().filter(function(u){return function(v){return a._line_config[v].unit===u.$jscomp$loop$prop$unit$32}}(d));console.log("creating y axis for unit "+d.$jscomp$loop$prop$unit$32+", index="+e+", codes="+JSON.stringify(g));g=g[0];var h=createSvgElement("g");h.setAttribute("id",f);h.setAttribute("fill",this._line_config[g].color);this._yAxisElement.appendChild(h);f=createSvgElement("text");h.appendChild(f);f.innerHTML=d.$jscomp$loop$prop$unit$32;f.style.fontSize=b+"px";f.setAttribute("y",
b);f.setAttribute("x",(4*e+.5)*b+"px");f=$jscomp.makeIterator(this._get_visible_min_max_for_unit(d.$jscomp$loop$prop$unit$32));var l=f.next().value;f=f.next().value;var k=1.2*b/(this._config.height/(f-l));f=0;var m=void 0;if(.5>k){f=1;for(m=k;.05>m;)f++,m*=10;m=.1>m?.1:.2>m?.2:.5;m/=Math.pow(10,f-1)}else m=Math.ceil(k);if(0===m){console.warn("something is wrong, i can feel it");break}k=this._config.height-b;for(l=Math.floor(l);this._value_to_y_coord(g,l)>k;)l+=m;var n=this._value_to_y_coord(g,l);
k="";for(var r=n,p=void 0,q=4*(e+.15)*b;n>2*b;){p=n;var t=createSvgElement("text");t.innerHTML=this.numberToDisplayText(l,f);t.setAttribute("y",n+.4*b);t.setAttribute("x",q+b/3);t.style.fontSize=b+"px";h.appendChild(t);k+="M "+q+" "+n+" h "+(b/-3).toFixed(2)+" ";l+=m;n=this._value_to_y_coord(g,l)}k+="M "+q+" "+p+" V "+r;f=createSvgElement("path");f.setAttribute("d",k);f.setAttribute("id","yLadder"+g);f.setAttribute("stroke",this._line_config[g].color);h.appendChild(f)}};
...
Wetplot.prototype._show_values_popup=function(a,b){var c=this,d=this._x_coords_to_seconds(a),e=document.getElementById("valuesPopupG"),f=document.getElementById("valuesPopupTime"),g=document.getElementById("valuesPopupBg"),h=function(u){return c._config.font_size_px*u};null==e?(e=createSvgElement("g"),e.setAttribute("id","valuesPopupG"),this._svgElement.appendChild(e),g=createSvgElement("path"),e.appendChild(g),g.style.fill=this._config.hover_box_background_color,g.style.fillOpacity=.75,g.setAttribute("id",
"valuesPopupBg"),f=createSvgElement("text"),f.setAttribute("id","valuesPopupTime"),f.style.fontSize=this._config.font_size_px+"px",e.appendChild(f)):e.style.display="inline";this.getVisibleLineCodes().forEach(function(u){u="valuesPopup"+u;var v=document.getElementById(u);null==v&&(v=createSvgElement("text"),v.style.fontSize=c._config.font_size_px+"px",v.classList.add("valueText"),v.setAttribute("id",u),e.appendChild(v))});var l=this.getVisibleLineCodes().filter(function(u){return"ybar"===c._line_config[u].type}),
k=document.getElementById("ybar-width-label");null===k&&0<l.length&&(k=createSvgElement("text"),k.style.fontSize=this._config.font_size_px+"px",k.style.fill="#333333",k.setAttribute("id","ybar-width-label"),e.appendChild(k));null!==k&&(0<l.length?(k.innerHTML=getText("YBAR_WIDTH")+": "+timedeltaSecondsToHumanReadable(this._config.ybar_width_seconds),k.style.display="inline"):(k.style.display="none",k.innerHTML=""));l=h(.25);var m=h(1),n=(new Date(1E3*d)).toLocaleString();n=n.substring(0,n.lastIndexOf(":"));
f.innerHTML=getText("TIME")+": "+n;f.setAttribute("y",m);n=Math.max(f.getBoundingClientRect().width,null===k?0:k.getBoundingClientRect().width);var r=this._seconds_to_values(d,!0);d=e.getElementsByClassName("valueText");for(var p=0;p<d.length;p++){var q=d.item(p),t=q.id.substring(11);this._line_config[t].visible?(q.innerHTML=this._line_config[t].name+": "+Math.round(100*r[t])/100+" "+this._line_config[t].unit,n=Math.max(n,q.getBoundingClientRect().width),q.style.fill=this._line_config[t].color,m+=
h(1),q.setAttribute("y",m),q.style.display="inline"):q.style.display="none"}null!==k&&"none"!==k.style.display&&(m+=h(1),k.setAttribute("y",m));n=Math.ceil(n+h(1));m+=h(.25);r=(this._config.height+m)/2;.5<b/this._config.width?(e.setAttribute("transform","translate("+(a-(n+h(1)))+", "+r+")"),g.setAttribute("transform","")):(e.setAttribute("transform","translate("+a+", "+r+")"),g.setAttribute("transform","rotate(180) translate("+-1*(n+h(1))+" "+-1*m+")"),l+=h(1));f.setAttribute("x",l);null!==k&&k.setAttribute("x",
l);for(a=0;a<d.length;a++)d.item(a).setAttribute("x",l);g.setAttribute("d","M 0 0 H "+n+" V "+.4*m+" L "+(n+h(1))+" "+.5*m+" L "+n+" "+.6*m+" V "+m+" H 0 V 0")};Wetplot.prototype._hide_values_popup=function(){var a=document.getElementById("valuesPopupG");null!==a&&(a.style.display="none")};Wetplot.prototype.getAllLineCodes=function(){return Object.getOwnPropertyNames(this._line_config).filter(function(a){return a!==DEFAULT_LINE_CODE})};Wetplot.prototype.getVisibleLineCodes=function(){var a=this;return this.getAllLineCodes().filter(function(b){return a._line_config[b].visible})};
...
Wetplot.prototype._seconds_to_values=function(a,b){var c=this;b=void 0===b?!1:b;var d=this._config.ybar_width_seconds,e=$jscomp.makeIterator(this._nearestTwoDataRows(a)),f=e.next().value,g=e.next().value,h={};if(null!==f||null!==g){null===f?f=g:null===g&&(g=f);e=this._data.heads.indexOf("Time");var l=f[e],k=(a-l)/(g[e]-l);a=l-l%d;var m=this._data.findNextBiggerValue(a),n=this._data.findNextBiggerValue(a+d);(b?this.getVisibleLineCodes():this.getAllLineCodes()).forEach(function(r){var p=c._data.heads.indexOf(r);
if("ybar"===c._line_config[r].type){for(var q=0,t=m;t<n;t++)q+=c._data.values[t][p];h[r]=q}else q=f[p],h[r]=q+(g[p]-q)*k})}return h};Wetplot.prototype.scrollTo=function(a){this._moveViewBoxPixels(a,!0)};Wetplot.prototype.zoomIn=function(){this._config.seconds_per_pixel/=2;this._config.seconds_per_grid_line/=2;this._config.ybar_width_seconds/=2;this._redraw_all()};
Wetplot.prototype.zoomOut=function(){this._config.seconds_per_pixel*=2;this._config.seconds_per_grid_line*=2;this._config.ybar_width_seconds*=2;this._redraw_all()};Wetplot.prototype.toggleLineVisibility=function(a){this._line_config[a].visible=!this._line_config[a].visible;this._redraw_all()};
...
WetplotData.prototype.findNextBiggerValue=function(a,b){var c=0,d=this.values.length;b=this.getColumnIndex(void 0===b?"Time":b);do{var e=Math.floor((c+d)/2);this.values[e][b]>a?d=e:c=e}while(1<d-c);return d};
